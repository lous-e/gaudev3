...
function ATopbar({ session = "deal_a93kf2" }) {
  return (
    <div style={{
      ...
      <span style={{ color: A.fg5 }}>$</span>
      <div style={{ flex: 1 }} />
      <APill tone="green">● live</APill>
      <span style={{ color: A.fg3 }}>round 3 / 3</span>
    </div>
  );
}
...
function AInspector() {
  const fields = [
    ["intent.id", "deal_a93kf2"],
    ["intent.item", "USB-C cable, 1m"],
    ["intent.qty", "1"],
    ["intent.target_price", "4.00 USDC"],
    ["intent.max_price", "5.00 USDC"],
    ["intent.max_rounds", "3"],
    ["policy.shim", "enforced"],
    ["policy.min_price", "4.20 USDC"],
    ["seller.handle", "cableworks.agent"],
    ["seller.pubkey", "0x9b21…7c8a"],
...
function AShell({ tab, setTab, children }) {
  return (
    <div style={{
